...
// SongCards rendern + Like-Delegation + globaler Sync
...
export function bindLikesDelegation(root = document) {
  ...
  root.addEventListener('click', async (ev) => {
    ...
    try {
      ...
      // Broadcast für Player + ALLE Cards
...
// --- Globaler Sync: Player- und Card-Updates auf ALLE passenden Cards spiegeln
document.addEventListener('gio:like-updated', (ev) => {
  const { song_id, liked, count } = ev.detail || {};
  if (!song_id) return;

  const cards = document.querySelectorAll(`.songcard[data-id="${song_id}"]`);
  cards.forEach((card) => {
    ...
    iconEl?.setAttribute('aria-pressed', String(!!liked));
    if (countEl && Number.isFinite(count)) countEl.textContent = String(Number(count));
  });
});
