...
async function playSong(song) {
  ...
  // Player-Like Count initialisieren und an Cards broadcasten
  refreshPlayerLike();
...
async function refreshPlayerLike() {
  ...
  try {
    ...
    const count = Number(map[String(current.song.id)] ?? 0);
    if (ui.likeCount) ui.likeCount.textContent = String(count);

    // Broadcast Count an alle Cards (liked-Status unbekannt → nur Zahl)
    document.dispatchEvent(new CustomEvent('gio:like-updated', {
      detail: { song_id: current.song.id, liked: undefined, count }
    }));
  } catch (_) { /* ignore */ }
}

/* --- Sync: Card-Updates auf Player spiegeln --- */
document.addEventListener('gio:like-updated', (ev) => {
  ...
  if (Number.isFinite(count) && ui.likeCount) ui.likeCount.textContent = String(Number(count));
