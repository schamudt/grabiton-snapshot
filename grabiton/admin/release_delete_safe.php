...
function url_with_query(string $base, array $params): string {
  $baseHasQuery = (strpos($base, '?') !== false);
  $qs = http_build_query(array_filter($params, fn($v)=>$v!==null && $v!==''));
  if ($qs === '') return $base;
  return $base . ($baseHasQuery ? '&' : '?') . $qs;
}

$releaseId = (int)($_GET['id'] ?? 0);
$artistId  = isset($_GET['artist']) ? (int)$_GET['artist'] : 0;
$from      = $_GET['from'] ?? ''; // 'overview' | 'list' | 'artist'
$from      = in_array($from, ['overview','list','artist'], true) ? $from : '';

if ($releaseId <= 0) {
  header('Location: ' . url_with_query('/grabiton/admin/releases_overview.php', ['err'=>'Ungültige Release-ID.']));
  exit;
}
...
$release = $st->fetch();
if (!$release) {
  header('Location: ' . url_with_query('/grabiton/admin/releases_overview.php', ['err'=>'Release nicht gefunden.']));
  exit;
}
...
$backArtistRelease = url_with_query('/grabiton/admin/releases.php', ['artist'=>$artistId,'from'=>'list']);
$back = ($from === 'overview') ? $backOverview : $backArtistRelease;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($confirm === 'yes') {
    try {
      ...
      $pdo->commit();
      header('Location: ' . url_with_query($back, ['msg' => 'Release „'.$release['title'].'“ wurde gelöscht.']));
      exit;
    } catch (Throwable $e) {
      if ($pdo->inTransaction()) $pdo->rollBack();
      header('Location: ' . url_with_query($back, ['err' => 'Löschen fehlgeschlagen: '.$e->getMessage()]));
      exit;
    }
  }
}
